...
const RENDER_SERVER_VERSION = "2026-05-14-audio-stitch";
...
function resolveFfprobeBinary() {
  const sibling = path.isAbsolute(FFMPEG_BIN)
...
if (SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY) {
  ...
  console.warn(
    "SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY missing; upload endpoints are disabled, but /stitch-final binary mode will still work.",
  );
}
...
function assertStorageConfigured() {
  if (!supabase) {
    throw new Error(
      "Storage upload is not configured on this render server. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY, or use /stitch-final with return_binary=true.",
    );
  }
}
...
async function downloadWithRetry(url, { attempts = 4, timeoutMs = 45000 } = {}) {
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    try {
      ...
      lastErr = new Error(`Download failed (${r.status})`);
...
app.get("/", (_req, res) =>
  res.json({
    ok: true,
    service: "truecosmic-ken-burns",
    version: RENDER_SERVER_VERSION,
    ffmpeg: FFMPEG_BIN,
    ffprobe: FFPROBE_BIN,
    storage_uploads: Boolean(supabase),
  }),
);
...
app.get("/health", async (_req, res) => {
  try {
    ...
    res.json({
      ok: true,
      service: "truecosmic-ken-burns",
      server_version: RENDER_SERVER_VERSION,
      ffmpeg: FFMPEG_BIN,
      ffprobe: FFPROBE_BIN,
...
app.post("/ken-burns", async (req, res) => {
  ...
  if (!image_url || !effect || (!return_binary && !output_filename)) {
    return res
      .status(400)
      .json({ error: "image_url, effect, and output_filename required unless return_binary=true" });
  }
  ...
  try {
    ...
    if (music_url) {
      try {
        ...
        if (m.ok) {
          fs.writeFileSync(musicPath, Buffer.from(await m.arrayBuffer()));
          hasMusic = true;
        }
      } catch (e) {
    ...
    const args = ["-y", "-loop", "1", "-framerate", "25", "-i", inPath];
    ...
    if (hasMusic) {
      args.push(
        "-filter_complex",
        `[1:a]volume=${music_volume},aloop=loop=-1:size=2e9[a]`,
        "-map",
        "0:v",
        "-map",
        "[a]",
...
app.post("/mix-audio", async (req, res) => {
  ...
  if (!video_url || (!return_binary && !output_filename)) {
    return res
      .status(400)
      .json({ error: "video_url and output_filename required unless return_binary=true" });
  }
  ...
  try {
    ...
    if (narration_url) {
      try {
        await dl(narration_url, narrPath);
        hasNarr = true;
      } catch (e) {
        console.warn("narr fetch", e.message);
      }
    }

    if (music_url) {
      try {
        await dl(music_url, musicPath);
        hasMusic = true;
      } catch (e) {
        console.warn("music fetch", e.message);
      }
    }
    ...
    if (Array.isArray(sound_effects)) {
      for (let i = 0; i < sound_effects.length; i++) {
        ...
        if (!sfx.url && !sfx.sound_effect_url && !sfx.audio_url) continue;

        const url = sfx.url || sfx.sound_effect_url || sfx.audio_url;
    ...
    if (mixLabels.length > 1) {
      filter =
        filterParts.join(";") +
        `;${mixLabels.join("")}amix=inputs=${mixLabels.length}:duration=first:dropout_transition=2:normalize=0[aout]`;
      mapAudio = "[aout]";
 ...
 * }
 *
 * Concatenates every clip/part in order, then mixes:
 * - narration as the main/front track
 * - sound effects at their timestamp/volume
 * - soundtrack looped underneath at low volume
 */
app.post("/stitch-final", async (req, res) => {
  ...
  if (clipUrls.length === 0 || (!return_binary && !output_filename)) {
    return res
      .status(400)
      .json({ error: "clip_urls (non-empty array) and output_filename required unless return_binary=true" });
  }
  ...
  try {
    ...
    const totalDuration =
      Math.max(1, durations.reduce((sum, d) => sum + (d || 0), 0) || localPaths.length * 5);
    ...
    const musicVolume = clampNumber(body.music_volume, 0.15, 0, 1);
    let musicPath = null;
      ...
      for (let i = 0; i < body.sound_effects.length; i++) {
        ...
        try {
          ...
          sfxPaths.push({
            ...
            volume: clampNumber(sfx.volume, 0.45, 0, 1),
          });
        } catch (e) {
          console.warn("sfx fetch", url, e.message);
        }
    ...
    const maxSfxEnd = sfxPaths.reduce(
      (max, s, i) => Math.max(max, s.timestamp + (sfxDurations[i] || 0)),
      0,
    );

    const finalDuration = Math.max(1, totalDuration, narrationDuration, maxSfxEnd);
    const videoPadDuration = Math.max(0, finalDuration - totalDuration);

    const inputs = [];

    for (const p of localPaths) {
      inputs.push("-fflags", "+genpts+discardcorrupt", "-err_detect", "ignore_err", "-i", p);
    }

    const narrationInputStart = localPaths.length;

    for (const p of narrationPaths) {
      inputs.push("-i", p);
    }
    ...
    parts.push(
      `[vcat]tpad=stop_mode=clone:stop_duration=${videoPadDuration},trim=duration=${finalDuration},setpts=PTS-STARTPTS[outv]`,
    );
    ...
    if (narrationPaths.length > 1) {
      ...
      for (let i = 0; i < narrationPaths.length; i++) {
        ...
        parts.push(
          `[${inputIdx}:a]aresample=44100,aformat=sample_rates=44100:channel_layouts=stereo[nprep${i}]`,
        );
        narrLabels.push(`[nprep${i}]`);
      }

      parts.push(
        `${narrLabels.join("")}concat=n=${narrationPaths.length}:v=0:a=1,volume=1.0[narr]`,
      );
      audioLabels.push("[narr]");
    } else if (narrationPaths.length === 1) {
      parts.push(
        `[${narrationInputStart}:a]aresample=44100,aformat=sample_rates=44100:channel_layouts=stereo,volume=1.0[narr]`,
      );
      audioLabels.push("[narr]");
    }
    ...
    if (musicInputIndex !== null) {
      parts.push(
        `[${musicInputIndex}:a]aresample=44100,aformat=sample_rates=44100:channel_layouts=stereo,aloop=loop=-1:size=2147483647,asetpts=N/SR/TB,volume=${musicVolume}[music]`,
      );
    ...
    sfxPaths.forEach((s, i) => {
      ...
      parts.push(
        `[${inputIdx}:a]aresample=44100,aformat=sample_rates=44100:channel_layouts=stereo,volume=${s.volume},adelay=${delayMs}:all=1[sfx${i}]`,
      );
    ...
    if (hasAudio) {
      ...
      parts.push(
        `[base]${audioLabels.join("")}amix=inputs=${audioLabels.length + 1}:duration=first:dropout_transition=0:normalize=0[aout]`,
      );
    }
...
app.listen(PORT, () => {
  console.log(`Ken Burns render server listening on :${PORT}; ffmpeg=${FFMPEG_BIN}`);
});
